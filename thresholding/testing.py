...
import datetime
import pickle
from model import MotionClassifier
...
raw1 = mne.io.read_raw_edf(os.getcwd() + '/parent/Accelerometer annotations/Version01_2_latest/Vers01_2_AF_Pos0deg/C3030260-221103-V01_2_Pos0deg.edf',
...
raw2 = mne.io.read_raw_edf(os.getcwd() + '/parent/Accelerometer annotations/Version01_2_latest/Vers01_2_AF_Pos180deg/C3030260-221103-V01_2_Pos180deg.edf',
                            include=['ACCX','ACCY', 'ACCZ'])
...
raw3 = mne.io.read_raw_edf(os.getcwd() + '/parent/Accelerometer annotations/Version01_2_latest/Vers01_2_AS_Pos0deg/C3090273-240103-Vers01_2_AS_Pos0deg.edf',
                            include=['ACCX','ACCY', 'ACCZ'])
## crop the recordings excluding the position changes
...
raw4 = mne.io.read_raw_edf(os.getcwd() + '/parent/Accelerometer annotations/Version01_2_latest/Vers01_2_AS_Pos180deg/C3080088-240103_Vers01_2_AS_Pos180deg.edf',
                           include=['ACCX','ACCY', 'ACCZ'])
## crop the recordings excluding the position changes
...
#classify epochs
train_activity_predictions, train_position_predictions = model.classify_motion(X_train)
#store in dataframe
df_train = pd.DataFrame({'gt_activity': [label for label in data1['act']],
                'gt_position':  [label for label in data1['pos']],
                'activity': train_activity_predictions,
                'position': train_position_predictions})

#smooth the predictions to remove small bursts of activity
df_train['activity'] = df_train['activity'].rolling(window=6, center=True).apply(lambda x: x.mode()[0])
...
#TESTING
#make predictions
test_activity_predictions, test_position_predictions = model.classify_motion(X_test)
df_test = pd.DataFrame({'gt_activity': [label for label in np.concatenate((data2['act'], data3['act'], data4['act']))],
                'gt_position': [label for label in np.concatenate((data2['pos'], data3['pos'], data4['pos']))],        
                'activity': test_activity_predictions,
                'position': test_position_predictions
                })

#smooth the predictions to remove small bursts of activity
df_test['activity'] = df_test['activity'].rolling(window=6, center=True).apply(lambda x: x.mode()[0])
